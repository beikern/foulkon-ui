...
import client.appstate.{GroupMemberFeedbackReporting, RemoveGroupFeedbackReporting}
...
object GroupMemberFeedbackSnackbar {

  case class Props(proxy: ModelProxy[Option[GroupMemberFeedbackReporting]])
  ...
  def apply(proxy: ModelProxy[Option[GroupMemberFeedbackReporting]]) = component(Props(proxy))
}
